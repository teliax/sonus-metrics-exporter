...
func GetSipStatisticsUrl(ctx MetricContext) string {
...
func ProcessSipStatistics(ctx MetricContext, xmlBody *[]byte, ch chan<- prometheus.Metric, result chan<- bool) {
	if len(*xmlBody) == 0 {
		result <- true
		return
	}
	...
	log.Info("SIP Statistics Metrics collected")
	result <- true
}
